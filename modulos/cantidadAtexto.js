...
const udc = (cantidad) => {
    ...
    return udc(cantidad,cantidadUDC);
}
...
const aTexto = (cantidad,pesos) => {
    ...
    str+=`${ c<1 ? centenas[1][0] : c>1 ? centenas[c] : d||u ? centenas[1][2]:centenas[1][1] }${d==1&&u<6?excepciones[u]:decenas[d]}${d>1&&u||(d==1&&u>5)?"y ":""}${d==1&&u<6 || (pesos && u==0) ?"":unidades[u]}`;
    if(c>0||d>0||u>0){
